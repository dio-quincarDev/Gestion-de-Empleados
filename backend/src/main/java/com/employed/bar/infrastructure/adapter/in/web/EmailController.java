...
public class EmailController {
    ...
    public EmailController(EmailService emailService, ReportingUseCase reportingUseCase, EmployeeRepositoryPort employeeRepository) {
    ...
    public ResponseEntity <String> sendWeeklyReports(
        ...
        ReportDto report = reportingUseCase.generateCompleteReport(startDateParsed,endDateParsed, employeeId);
        String emailBody = emailService.generateEmailBody(report, employee);
    ...
    public ResponseEntity<String> sendTestBulkEmails(
        ...
        List<CompletableFuture<Void>> emailFutures = employees.stream()
                .map(employee -> {
                    ...
                    ReportDto report = reportingUseCase.generateCompleteReport(date, date, employee.getId());
                    return emailService.sendEmailAsync(employee, report);
                })
                .collect(Collectors.toList());
